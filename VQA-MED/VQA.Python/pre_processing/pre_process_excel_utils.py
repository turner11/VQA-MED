...
if __name__ == '__main__':
    ...
    path = args.path #or dbg_file_csv_train

    def single_process(data_path, destintation=None):
        destination =  destintation or os.path.splitext(data_path)[0] + '_post_pre_process.xlsx'
        DataPreProcessor.preprocessed_data__to_processed_excel(data_path, destination)
    if path:
        single_process(path, args.destination)
    else:
